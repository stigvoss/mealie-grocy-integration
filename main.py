...
if __name__ == "__main__":
    ...
    with (
        ...
        products_user_field = grocy.get_user_field("mealieId", "products")

        if products_user_field is None:
            grocy.create_mealie_field("mealieId", "products")

        recipes_user_field = grocy.get_user_field("mealieId", "recipes")

        if recipes_user_field is None:
            grocy.create_mealie_field("mealieId", "recipes")
        ...
        for mealie_recipe in mealie_recipies:
            ...
            for ingredient in mealie_recipe["recipeIngredient"]:
                ...
                grocy_ingredient = grocy.get_recipe_ingredient(
                    grocy_recipe["id"], grocy_product["id"]
                )
