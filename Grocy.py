...
class Client:
    ...
    def get_user_field(self, name: str, entity_type: str) -> UserField | None:
        data: list[UserField] = (
            self.client.get(
                "/api/objects/userfields",
                params={"query[]": [f"name={name}", f"entity={entity_type}"]},
            )
            .raise_for_status()
            .json()
        )
    ...
    def get_recipe_by_mealie_id(self, mealie_id: str) -> Recipe | None:
        data: list[Recipe] = (
            self.client.get("/api/objects/recipes").raise_for_status().json()
        )
        for recipe in data:
    ...
    def get_product_by_mealie_id(self, mealie_id: str) -> Product | None:
        data: list[Product] = (
            self.client.get("/api/objects/products").raise_for_status().json()
        )
        for product in data:
    ...
    def get_recipe_ingredient(
        ...
        for ingredient in data:
            if (
                ...
                and ingredient["product_id"] == product_id
            ):
                return ingredient
